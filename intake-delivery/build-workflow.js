...
const HAS_LOCAL = fs.existsSync(CFG_LOCAL);
const CFG = JSON.parse(fs.readFileSync(
  HAS_LOCAL ? CFG_LOCAL : path.join(__dirname, 'config.example.json'),
  'utf8'
));
...
codeNode('Generate DOCX', path.join('..', 'thesis-chapters', 'docx-generator.js'), [2860, 120]);
...
// A build from config.local.json carries a real Sheet id, the live sub-workflow id
// and per-instance credential ids, so it goes to the gitignored workflow.local.json.
// The committed workflow.json is built from config.example.json and is importable
// anywhere: placeholder ids, no credentials blocks, attach them in the n8n UI.
const OUT = path.join(__dirname, HAS_LOCAL ? 'workflow.local.json' : 'workflow.json');
